...
// Профиль высот для dubossary
const dubossaryCoordinates = [
  { lat: 47.28140096947098, lng: 29.131151783856854, name: 'Парк Энергетиков' },
  { lat: 47.27947021365455, lng: 29.12609697958752, name: 'Пляжная зона' },
  { lat: 47.278036292550354, lng: 29.123779550955906, name: 'ГЭС' }
];

async function createDubossaryElevationChart() {
  const elevationData = await fetchElevationData(dubossaryCoordinates);
  ...
  const ctx = document.getElementById('dubossaryElevationChart');
  if (!ctx) return;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', createDubossaryElevationChart);
} else {
  createDubossaryElevationChart();
}
