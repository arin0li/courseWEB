...
// Функции для профиля высот (общие)
function calculateDistance(lat1, lon1, lat2, lon2) {
  const R = 6371;
  const dLat = (lat2 - lat1) * Math.PI / 180;
  const dLon = (lon2 - lon1) * Math.PI / 180;
...
// Профиль высот для greenTiraspol
const greenTiraspolCoordinates = [
  { lat: 46.837924573343095, lng: 29.61081046970385, name: 'Екатерининский парк' },
  { lat: 46.830936661045904, lng: 29.642463463158062, name: 'Ботанический сад' }
];

async function createGreenTiraspolElevationChart() {
  const elevationData = await fetchElevationData(greenTiraspolCoordinates);
  ...
  const ctx = document.getElementById('greenTiraspolElevationChart');
  if (!ctx) return;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', createGreenTiraspolElevationChart);
} else {
  createGreenTiraspolElevationChart();
}
