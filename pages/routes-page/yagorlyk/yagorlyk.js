...
// Профиль высот для yagorlyk (agorlyk)
const yagorlykCoordinates = [
  { lat: 47.38118978270089, lng: 29.20021935439449, name: 'Обзорная площадка (между Гоянами и Дойбанами)' },
  { lat: 47.38836492771726, lng: 29.17590761515721, name: 'Обзорная площадка на заповедник Ягорлык' },
  { lat: 47.3937107359394, lng: 29.19238710617221, name: 'Ягорлык' },
  { lat: 47.372346284005594, lng: 29.16720840967653, name: 'Кемпинг' },
  { lat: 47.327439316401886, lng: 29.118110650981077, name: 'Обзор на село Роги' }
];

async function createYagorlykElevationChart() {
  const elevationData = await fetchElevationData(yagorlykCoordinates);
  ...
  const ctx = document.getElementById('yagorlykElevationChart');
  if (!ctx) return;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', createYagorlykElevationChart);
} else {
  createYagorlykElevationChart();
}
